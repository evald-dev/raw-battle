...
export default function TasayBattle() {
  const bgVideoRef     = useRef(null);
  const screenVideoRef = useRef(null);
  const filmScrollRef  = useRef(null);

  const [panelOpen,   setPanelOpen]   = useState(false);
  const [filmContent, setFilmContent] = useState(null); // jsx
  const [modalOpen,   setModalOpen]   = useState(false);
  const [modalContent, setModalContent] = useState(null); // jsx
...
Лучший трек раунда - 30€`
          ...
          />
          <span className="taster-label" style={{ "--x": "13.6%", "--y": "64.5%" }}>призовые</span>
