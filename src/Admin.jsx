    ...
    <>
      ...
      <div className="top-marquee" aria-hidden="true">
        <div className="top-marquee-track">
          0.49 GERMANY • СЫРОЙ БАТЛ • ADMIN PANEL • 0.49 GERMANY • СЫРОЙ БАТЛ •
          ADMIN PANEL •
        </div>
      </div>
      ...
      <main className="admin-layout">
        <div className="admin-topbar">
          <button className="tabelle-back" onClick={() => navigate("/")}>
            ← назад
          </button>
...
function RoundsTab() {
  ...
  async function loadRounds() {
    setLoading(true);
    const [{ data: roundData }, { data: judgeData }, { data: rjData }] =
      await Promise.all([
  ...
  async function toggleJudge(roundId, judgeId) {
    ...
    setRoundJudges((prev) => {
      ...
      if (assigned) set.delete(judgeId);
      else set.add(judgeId);
      map[roundId] = set;
      return map;
    });
    if (assigned) {
      ...
      await supabase
        .from("round_judges")
        .insert({ round_id: roundId, judge_id: judgeId });
    }
  }
    ...
    <div className="admin-section">
      <div className="admin-form">
        <div className="admin-form-title">
          {editing ? "Редактировать раунд" : "Новый раунд"}
        </div>
        ...
        <div className="admin-form-row">
          ...
          <textarea
            ...
            onChange={(e) =>
              setForm((p) => ({ ...p, extra_info: e.target.value }))
            }
            rows={3}
          />
        </div>
        <div className="admin-form-row">
          ...
          <input
            ...
            onChange={(e) =>
              setForm((p) => ({ ...p, visual_url: e.target.value }))
            }
          />
        </div>
              ...
              >
                ...
                {isOpen && (
                  <div className="admin-form" style={{ gap: 12 }}>
                    <div className="admin-form-title">
                      Судьи в раунде «{r.name}»
                    </div>
                    {judges.length === 0 ? (
                      <div className="admin-list-meta">
                        Сначала добавьте судей во вкладке «Судьи»
                      </div>
                    ) : (
                      <div
...
function ParticipantsTab() {
  ...
  useEffect(() => {
    if (activeRound) loadParticipants(activeRound);
  }, [activeRound]);
  ...
  async function remove(id) {
    if (!confirm("Удалить участника? Все его оценки также будут удалены."))
      return;
  ...
  async function moveUp(idx) {
    if (idx === 0) return;
    const a = participants[idx],
      b = participants[idx - 1];
    await supabase
      .from("participants")
      .update({ order_num: b.order_num })
      .eq("id", a.id);
              ...
              <div className="admin-list-info">
                {editing === p.id ? (
                  <div className="admin-inline-edit">
                    ...
                    />
                    <button
                      className="admin-btn-save small"
                      onClick={() => saveEdit(p.id)}
                    >
                      ✓
                    </button>
                    <button
                      className="admin-btn-cancel small"
...
function JudgesTab() {
  ...
  return (
    <div className="admin-section">
      <div className="admin-form">
        <div className="admin-form-title">
          {editing ? "Редактировать судью" : "Новый судья"}
        </div>
        <div className="admin-form-row">
        ...
        <div className="admin-list">
          {judges.map((j, idx) => (
            <div className="admin-list-item" key={j.id}>
              ...
              {j.avatar_url ? (
                <img
                  className="admin-judge-avatar"
                  src={j.avatar_url}
                  alt={j.name}
                />
              ) : (
                <div className="admin-judge-avatar-placeholder">
                  {j.name[0]}
                </div>
              )}
              <div className="admin-list-info">
...
function UsersTab() {
  ...
  async function createUser() {
    ...
    if (form.role === "judge" && !form.judge_id) {
      ...
      <div className="admin-form">
        ...
        <div className="admin-form-row">
          ...
          <select
            ...
            value={form.role}
            onChange={(e) =>
              setForm((p) => ({ ...p, role: e.target.value, judge_id: "" }))
            }
          >
        ...
        {form.role === "judge" && (
          <div className="admin-form-row">
            ...
            <select
              ...
              value={form.judge_id}
              onChange={(e) =>
                setForm((p) => ({ ...p, judge_id: e.target.value }))
              }
            >
        ...
        {error && (
          <div className="font-[Montserrat] text-[12px] text-[#d94b6a]">
            {error}
          </div>
        )}
...
function VisibilityTab() {
  ...
  async function toggleVisibility(roundId, current) {
    ...
    await supabase
      ...
      .eq("id", roundId);
    setRounds((prev) =>
      prev.map((r) => (r.id === roundId ? { ...r, is_public: !current } : r)),
    );
    setSaving(null);
  }
  ...
  return (
    <div className="admin-section">
      <div className="admin-form">
        ...
        >
          Включите видимость раунда чтобы публичная таблица показывала
          результаты для всех пользователей (без входа в систему).
        </p>
      </div>
