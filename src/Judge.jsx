...
export default function Judge() {
  ...
  useEffect(() => { if (judgeId) loadRounds(); }, [judgeId]);
  ...
  async function saveFavorite(rank, participantId) {
    ...
    if (current === participantId) {
      ...
      await supabase.from("favorites").delete()
        .eq("round_id", activeRound).eq("judge_id", judgeId).eq("rank", rank);
      return;
    }
    setFavorites(prev => ({ ...prev, [rank]: participantId }));
    await supabase.from("favorites").upsert({
      round_id: activeRound, judge_id: judgeId, participant_id: participantId, rank,
    ...
    <>
      ...
      <main className="relative z-10 max-w-[860px] mx-auto px-4 pt-20 pb-20">
        ...
        {loading ? (
          ...
          <>
            {/* ── Fortschritt ── */}
            <div className={`${BG} border border-white/[0.12] rounded-xl p-4 mb-6 flex items-center gap-4`}>
              <div className="flex-1">
                ...
                <div className="h-2 bg-white/[0.08] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${participants.length ? (scoredCount / participants.length) * 100 : 0}%` }}
                  />
                </div>
              </div>
            ...
            <div className={`${BG} border border-white/[0.12] rounded-xl p-5`}>
              <div className="font-[Montserrat] text-[11px] tracking-[0.18em] uppercase text-[rgba(245,232,207,0.35)] mb-4">
                Мои фавориты (независимо от баллов)
              </div>
              <div className="flex flex-col gap-3">
                {[1, 2, 3].map(rank => {
                  ...
                  return (
                    <div key={rank} className="flex items-center gap-3 flex-wrap">
                      <div className={`w-7 h-7 rounded-full border ${c.border} ${c.bg} flex items-center justify-center font-[Montserrat] text-[12px] font-bold ${c.text} flex-shrink-0`}>
                        {rank}
                      </div>
            ...
            {allScored && (
              <div className="mt-6">
                ...
                <div className="flex flex-col gap-4">
                  {judges.map(j => (
                    <div key={j.id} className={`${BG} border border-white/[0.12] rounded-xl p-4`}>
                      <div className="flex items-center gap-2 mb-3">
                        {j.avatar_url
                          ? <img src={j.avatar_url} alt={j.name} className="w-7 h-7 rounded-full object-cover border border-[#d94b6a]" />
                          : <div className="w-7 h-7 rounded-full bg-[rgba(217,75,106,0.15)] border border-[rgba(217,75,106,0.3)] flex items-center justify-center text-[11px] font-bold text-[#d94b6a]">{j.name[0]}</div>
                        }
                        <div className="font-[Montserrat] text-[12px] font-bold text-[#f5e8cf] tracking-[0.06em] uppercase">{j.name}</div>
                      </div>
                      <div className="flex gap-3 flex-wrap">
                        {[1,2,3].map(rank => {
                          ...
                          const c = RANK_COLORS[rank];
                          return (
                            <div key={rank} className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${c.border} ${c.bg}`}>
      ...
      {commentModal && (
        <>
          <div onClick={() => setCommentModal(null)} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200]" />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[201] bg-[rgba(10,10,12,0.98)] border border-white/[0.12] rounded-xl p-7 w-[min(92vw,440px)]">
            <div className="font-[Montserrat] text-[13px] font-bold tracking-[0.1em] uppercase text-[#f5e8cf] mb-4">Комментарий</div>
            <textarea
            ...
            <div className="flex gap-2.5 justify-end">
              <button onClick={() => setCommentModal(null)} className={`px-4 py-2 rounded-full border border-white/[0.12] ${BG} text-[rgba(245,232,207,0.5)] font-[Montserrat] text-[11px] font-bold tracking-[0.1em] uppercase cursor-pointer transition-all hover:text-[#f5e8cf]`}>Отмена</button>
              <button onClick={() => saveComment(commentModal.participantId, commentModal.value)} className="px-4 py-2 rounded-full border border-[#d94b6a] bg-[rgba(217,75,106,0.15)] text-[#f5e8cf] font-[Montserrat] text-[11px] font-bold tracking-[0.1em] uppercase cursor-pointer transition-all hover:bg-[rgba(217,75,106,0.3)]">Сохранить</button>
            </div>
          </div>
        </>
      )}
      ...
      <footer className="site-footer">
        <div className="site-footer-links">
          <button className="footer-modal-link" type="button" onClick={() => navigate("/")}>← Самплер</button>
        </div>
      </footer>
    </>
  );
